...
class QtColorAnnotationSettingsWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.btn_add_all = QPushButton("Add all >>") # added
        self.btn_add_all.clicked.connect(self.addLabels)
    ...
    def addLabel(self):
        if self.selectedL is not None and self.selectedL in self.label_nameL:
    ...
    def addLabels(self): # added
        L = len(self.label_nameL)
        for i in range(L):
            self.selectedL = self.label_nameL[0]
            self.addLabel()
